...
def get_prefix_for_submission(prefix: str, mode: str, limit: int = 10) -> str:
    """Process prefix written to the submission (not used for retrieval)."""
    ...
    return "\n".join(list(dict.fromkeys(import_lines + tail_lines)))


def get_retrieval_prefix(prefix: str, limit: int = 20) -> str:
    """All import lines + last `limit` non-import lines — used as retrieval query."""
    lines        = prefix.splitlines()
    import_lines = [l for l in lines if l.startswith(("import ", "from "))]
...
def get_embedding_query(prefix: str, suffix: str) -> str:
    """
    Embedding query for chunk scoring:
    code-only prefix tail (no imports) + filtered suffix.
    Focused on *what the code is doing*, not its dependency graph.
    """
    lines      = prefix.splitlines()
...
def collect_best_context(
    ...
    # ------------------------------------------------------------------
    # 1. Score every file on all three signals
    # ------------------------------------------------------------------
    pd_scores:  list[tuple[float, str]] = []
    iou_scores: list[tuple[float, str]] = []
    rec_scores: list[tuple[float, str]] = []
    ...
    for fp in all_files:
        ...
        # Path distance (lower = better → invert)
        if import_files:
            ref  = min(import_files, key=lambda p: path_distance(fp, p))
            dist = path_distance(fp, ref)
        else:
            dist = fp.count(os.sep) - root_dir.count(os.sep)
        pd_scores.append((dist, fp))

        # Lines IoU
        iou_scores.append((lines_iou(query_text, content), fp))

        # Recency
        rec_scores.append((1.0 if fp in recent_set else 0.0, fp))

    def normalize(scores: list[tuple[float, str]], invert: bool = False) -> list[tuple[float, str]]:
        vals = [s for s, _ in scores]
        mn, mx = min(vals), max(vals)
        rng = mx - mn or 1.0
        result = [((mx - s) / rng if invert else (s - mn) / rng, fp)
                  for s, fp in scores]
        return result

    pd_norm  = normalize(pd_scores,  invert=True)   # smaller dist → higher score
    iou_norm = normalize(iou_scores, invert=False)
    rec_norm = normalize(rec_scores, invert=False)

    # Fuse: path distance 40%, IoU 40%, recency 20%
    fused = rank_fusion([pd_norm, iou_norm, rec_norm], [0.40, 0.40, 0.20])

    # Pin import-resolved files (score = 2.0 > any fused score)
    for fp in import_files:
        if fp in file_contents:
            fused[fp] = 2.0

    # Sort ascending (least → most relevant); take top candidates for chunk scan
    sorted_files = sorted(fused.items(), key=lambda x: x[1])
    # Candidates for chunk scoring: top max_files*3 most relevant
    candidates = [fp for fp, _ in sorted_files if fp in file_contents]
    candidates_top = candidates[-(max_files * 3):]   # last N = highest fused score
        ...
        for datapoint in reader:
            ...
            if strategy == "random":
                ...
                fn      = find_bm25_file(root_directory,
                                         get_retrieval_prefix(raw_prefix),
