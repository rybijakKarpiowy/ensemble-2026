...
def build_submission_prefix(prefix: str, root_dir: str, limit: int = 50) -> str:
    # 50 lines keeps the function signature, class header, and nearby context
    # visible to the model without wasting too much of the context window.
    # The full raw prefix is used when it is shorter than the limit.
...
def build_submission_suffix(suffix: str) -> str:
    # The evaluator does NOT trim the suffix — only the context is trimmed from
    # the left. Sending the full suffix preserves all FIM signal for the model.
    return suffix
...
def bm25_scores(query: str, file_contents: dict[str, str]) -> dict[str, float]:
    ...
    # Normalise by 95th-percentile score rather than the max.
    # A single outlier file (near-duplicate of the query) would otherwise
    # compress all other scores into [0, ~0.1], making proximity and boost
    # multipliers almost irrelevant for the rest of the corpus.
    sorted_vals = sorted(raw)
    p95_idx     = max(0, int(len(sorted_vals) * 0.95) - 1)
    p95         = sorted_vals[p95_idx] or 1.0
    return {fps[i]: min(float(raw[i]) / p95, 1.0) for i in range(len(fps))}
...
def _path_dist(a: str, b: str) -> int:
    pa = a.replace("\\", "/").split("/")
    pb = b.replace("\\", "/").split("/")
    # Count the longest common prefix (directory components)
    c = 0
...
def _chunk_by_functions(content: str, max_lines: int = 60,
    ...
    while i < len(lines):
        s = lines[i].strip()
        # Start of a decorator stack: walk forward past all consecutive
        # decorator lines to find whether they lead to a def/class.
        if s.startswith("@"):
            j = i
            while j < len(lines) and lines[j].strip().startswith("@"):
                j += 1
            # j now points to the line after all decorators
            if j < len(lines) and _is_definition_line(lines[j]):
                # Start the chunk at the first decorator, not the def line
                if i - boundaries[-1] >= min_lines:
                    boundaries.append(i)
                i = j + 1
                continue
        elif _is_definition_line(lines[i]):
            if i - boundaries[-1] >= min_lines:
                boundaries.append(i)
        i += 1
...
def dedup_chunks(chunks: list[str], threshold: float = 0.6) -> list[str]:
    ...
    for chunk in chunks:
        ...
        if not cset:
            continue  # skip empty chunks entirely
        dominated = False
        for p in kept_sets:
            union = cset | p
            if not union:
                continue
            if len(cset & p) / len(union) >= threshold:
                dominated = True
                break
        if not dominated:
            kept.append(chunk)
...
def collect_best_context(
    ...
    """
    File scoring (uniform formula for all files):
      fused = BM25(file, query) x path_proximity x boost
        path_proximity = 1/(dist+1), dist measured from inferred completion anchor
        boost: recent files x2.0, import-resolved x1.5, others x1.0
      A recent or imported file with BM25=0 still scores 0 -- no free rides.
      All scores on the same scale so chunk-score normalisation is meaningful.

    Chunk scoring:
      Function-boundary chunking -> embedded with code model -> top-K per file
      Chunk and file scores independently normalised to [0,1] then combined
        final = 0.6 * norm_chunk_emb + 0.4 * norm_file_fused
      Overlapping chunks deduplicated

    Assembly:
      Least relevant first, most relevant last (survives left-trim on Mellum 8K)
    """
    ...
    # Phase 1: BM25 scoring — tokenise only (no full content load yet).
    # This avoids loading every .py file into RAM for large repos.
    tok_corpus: dict[str, list[str]] = {}
    for fp in all_files:
        try:
            with open(fp, encoding="utf-8") as f:
                tok_corpus[fp] = _tok(f.read())
        except Exception:
            pass
    if not tok_corpus:
        return ""
    ...
    fps    = list(tok_corpus)
    if q_toks:
        raw_bm25   = BM25Okapi([tok_corpus[fp] for fp in fps]).get_scores(q_toks)
        sorted_vals = sorted(raw_bm25)
        p95_idx     = max(0, int(len(sorted_vals) * 0.95) - 1)
        p95         = sorted_vals[p95_idx] or 1.0
        bm25_map    = {fps[i]: min(float(raw_bm25[i]) / p95, 1.0)
                       for i in range(len(fps))}
    else:
        bm25_map = {fp: 0.0 for fp in fps}

    # Score every file — uniform formula: BM25 x path_proximity x boost
    #   path_proximity = 1/(dist+1), boost: recent x2.0, imports x1.5, else x1.0
    fused: dict[str, float] = {}
    for fp in fps:
        b = bm25_map.get(fp, 0.0)
        ...
        fused[fp] = b * proximity * boost

    # Phase 2: load full content only for top candidates
    sorted_fps     = sorted(fused, key=lambda fp: fused[fp])
    candidates_top = sorted_fps[-(max_files * 3):]
    ...
    # When all values are identical the range is 0 — return uniform 1.0s
    # so every chunk is treated equally and order is not arbitrary.
    def _norm(vals):
...
predictions_file       = os.path.join("predictions",
                                       f"{language}-{stage}-ens_weight.jsonl")
